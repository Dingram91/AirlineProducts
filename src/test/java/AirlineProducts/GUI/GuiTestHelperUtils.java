...
public class GuiTestHelperUtils {
    ...
    /**
     * Calculates the relative luminance (brightness based on the human eye)
     *
     * @param c
     * @return
     */
    static double calculateColorLuminance(Color c) {
        ...
        double r = (normRed <= 0.03928) ? (normRed * 12.92)
                : (Math.pow((normRed + 0.055) / 1.055, 2.4));
        double g = (normGreen <= 0.03928) ? (normGreen * 12.92)
                : (Math.pow((normGreen + 0.055) / 1.055, 2.4));
        double b = (normBlue <= 0.03928) ? (normBlue * 12.92)
                : (Math.pow((normBlue + 0.055) / 1.055, 2.4));
        return (0.2126 * r) + (0.7152 * g) + (0.0722 * b);
    }

    /**
     * Calculates the contrast for 2 different colors.
     *
     * @param c1 Color
     * @param c2 Color
